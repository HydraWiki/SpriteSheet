(function(spriteCow) {
	...
	(function() {
		spriteCow.initialize = function() {
			...
			var $canvasContainer  = $('#spritecow');
			var $codeContainer    = $('#spritecow_preview');
			...
			spriteCanvasView.bind('bgColorHover', function(color) {
				toolbarTop.feedback(spriteCow.colorBytesToCss(color));
			});
			...
			spriteCanvasView.bind('bgColorSelect', function(color) {
				...
				toolbarTop.feedback( 'Background set to ' + spriteCow.colorBytesToCss(color) );
			});
		...
		spriteCow.colorBytesToCss = function(color) {
			if (color[3] === 0) {
				return 'transparent';
			}
